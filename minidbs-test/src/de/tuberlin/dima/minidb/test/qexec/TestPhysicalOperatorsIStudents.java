...
{	
	 ...
	 */
	protected String configFileName = this.getClass().getResource("/config.xml").getPath();
	...
	/**
	 * Tests the delete operator using different page sizes.
	 */
//	@Test
	public void testDeleteOperator() throws Exception
	{
		...
		for (int pi = 0; pi < PageSize.values().length; pi++){
			...
			for (int si = 0; si < NUM_SCHEMAS; si++)
			{
				...
				// generate a random set of columns
				int numCols = random.nextInt(8) + 1;
				int[] producedColumnIndexes = new int[numCols];
				
				// create the columns as given
				for (int col = 0; col < numCols; col++) {
					DataType type = getRandomDataType(true);
					schema.addColumn(ColumnSchema.createColumnSchema("COL" + col, type, true, false));
					producedColumnIndexes[col] = col;
				}
				...
				{				
					...
					HashSet<DataTuple> remainingTuples = new HashSet<DataTuple>(); 
					ArrayList<DataTuple> toDelete = new ArrayList<DataTuple>();
					...
					for (int i = 0; i < numValuesToInsert; i++) 
					{
						...
						// add tuple to insertion list or to the table page
						if(random.nextBoolean() && toDelete.size() < 10000)
						{
							// mark tuple for later insertion into the table
							RID lastInserted = new RID(currentPage.getPageNumber(), currentPage.getNumRecordsOnPage() - 1);
							toDelete.add(new DataTuple(new DataField[] { lastInserted } ));
						}
						else
						{
							// remember all tuples that won't be deleted
							remainingTuples.add(tuple);							
						}
					}
					// unpin page
					bufferPool.unpinPage(TABLE_RESOURCE_ID, currentPage.getPageNumber());
					...
					// create mock plan operator
					MockPlanOperator mock = new MockPlanOperator(toDelete);
					// create the delete operator
					DeleteOperator delete = AbstractExtensionFactory.getExtensionFactory().createDeleteOperator(bufferPool, TABLE_RESOURCE_ID, mock);
					
					// open the delete
					delete.open(null);
					
					Set<DataTuple> result = new HashSet<DataTuple>();
					
					// go over all tuples
					DataTuple nextTuple = null;
					while ((nextTuple = delete.next()) != null) 
					{
						result.add(nextTuple);
					}
					...
					assertTrue("The delete operator should return only one big int field.", result.size() == 1 && (((DataTuple) result.toArray()[0]).getField(0) instanceof BigIntField));
					assertTrue("The number of tuples returned from the insert operator does not match the number of inserted tuples", ((BigIntField)((DataTuple) result.toArray()[0]).getField(0)).getValue() == toDelete.size());
					
					// scan all tuples 
					TableScanOperator scan = AbstractExtensionFactory.getExtensionFactory().createTableScanOperator(bufferPool, resManager, TABLE_RESOURCE_ID, producedColumnIndexes, new LowLevelPredicate[0], 4);
					
					scan.open(null);
					
					HashSet<DataTuple> containedTuples = new HashSet<DataTuple>();
					while((nextTuple = scan.next()) != null)
					{
						containedTuples.add(nextTuple);
					}
					
					// close the scan
					scan.close();
					
					assertTrue("Not all values that should be contained in the table scan result are contained.", containedTuples.containsAll(remainingTuples));
					assertTrue("The table scan operator returned values that should not have been returned.", remainingTuples.containsAll(containedTuples));
					
					// help the GC
					result.clear();
					remainingTuples.clear();
					toDelete.clear();
					containedTuples.clear();
					// clean up
					bufferPool.closeBufferPool();
					bufferPool = null;
					resManager.closeResource();
	...
	/**
	 * Tests the insert operator using different page sizes.
	 */
//	@Test
	public void testInsertOperator() throws Exception
	{		
		for (int pi = 0; pi < PageSize.values().length; pi++)
		{
			PageSize ps = PageSize.values()[pi];
			...
			for (int si = 0; si < NUM_SCHEMAS; si++)
			{
				...
				BufferPoolManager bufferPool = null;
				TableResourceManager resManager = null;
				IndexResourceManager[] ixManagers = new IndexResourceManager[this.indexFile.length];
				IndexSchema[] ixSchemas = new IndexSchema[this.indexFile.length];
				BTreeIndex[] indexes = new BTreeIndex[this.indexFile.length];
				int[] columnNumbers = new int[this.indexFile.length];
				try 
				{				
					// make sure the table file does not exist
					try 
					...
					for(int i = 0; i < this.indexFile.length; i++)
					{
						try 
						{
							IndexResourceManager.deleteIndex(this.indexFile[i]);
						}
						catch (IOException ioex) {}

						ixSchemas[i] = new IndexSchema(schema, random.nextInt(schema.getNumberOfColumns()));
						ixManagers[i] = IndexResourceManager.createIndex(this.indexFile[i], ixSchemas[i]);
						bufferPool.registerResource(INDEX_RESOURCE_ID + i, ixManagers[i]);
						indexes[i] = AbstractExtensionFactory.getExtensionFactory().createBTreeIndex(ixSchemas[i], bufferPool, INDEX_RESOURCE_ID + i);
						columnNumbers[i] = ixSchemas[i].getColumnNumber();
					}
					
					ArrayList<DataTuple> allTuples = new ArrayList<DataTuple>();
					ArrayList<DataTuple> toInsert = new ArrayList<DataTuple>();
					ArrayList<ArrayList<DataField>> indexValues = new ArrayList<ArrayList<DataField>>();
					for(int i = 0; i < this.indexFile.length; i++)
					{
						indexValues.add(i, new ArrayList<DataField>());
					}
					...
					for (int i = 0; i < numValuesToInsert; i++) 
					{
						...
						else
						{
							// mark tuple for later insertion into the table
							toInsert.add(tuple);
							for(int x = 0; x < this.indexFile.length; x++)
							{
								indexValues.get(x).add(tuple.getField(columnNumbers[x]));
							}
						}
						// remember all tuples
						allTuples.add(tuple);
					...
					for(int i = 0; i < this.indexFile.length; i++)
					{
						ixManagers[i] = IndexResourceManager.openIndex(this.indexFile[i], schema);
						ixSchemas[i] = ixManagers[i].getSchema();
						bufferPool.registerResource(INDEX_RESOURCE_ID + i, ixManagers[i]);
						indexes[i] = AbstractExtensionFactory.getExtensionFactory().createBTreeIndex(ixSchemas[i], bufferPool, INDEX_RESOURCE_ID + i);
					}
					...
					// create mock plan operator
					MockPlanOperator mock = new MockPlanOperator(toInsert);
					// create the insert Operator
					InsertOperator insert = AbstractExtensionFactory.getExtensionFactory().createInsertOperator(bufferPool, resManager, TABLE_RESOURCE_ID, indexes, columnNumbers, mock);
					...
					// go over all tuples
					DataTuple nextTuple = null;
					while ((nextTuple = insert.next()) != null) 
					{
						result.add(nextTuple);
					}
					// close the scan
					...
					// check post conditions
					assertTrue("The insert operator should return only one big int field.", result.size() == 1 && (((DataTuple) result.toArray()[0]).getField(0) instanceof BigIntField));
					assertTrue("The number of tuples returned from the insert operator does not match the number of inserted tuples", ((BigIntField)((DataTuple) result.toArray()[0]).getField(0)).getValue() == toInsert.size());
					
					// check that tuples were inserted into table					
					// scan all tuples 
					TableScanOperator scan = AbstractExtensionFactory.getExtensionFactory().createTableScanOperator(bufferPool, resManager, TABLE_RESOURCE_ID, producedColumnIndexes, new LowLevelPredicate[0], 4);
					...
					assertTrue("Not all values that should be contained in the table scan result are contained.", containedTuples.containsAll(allTuples));
					assertTrue("The table scan operator returned values that should not have been returned.", allTuples.containsAll(containedTuples));
					
					for(int i = 0; i < indexes.length; i++)
					{
						// sort list to be able to easily get the smallest and biggest key
						Collections.sort(indexValues.get(i));
						// retrieve all keys of the index
						IndexResultIterator<DataField> it = indexes[i].lookupKeys(indexValues.get(i).get(0), indexValues.get(i).get(indexValues.get(i).size()-1), true, true);
						ArrayList<DataField> ixResult = new ArrayList<DataField>();
						while(it.hasNext())
						{
							ixResult.add(it.next());
						}
						// check that the tuples were inserted into the indexes
						assertTrue("Not all values that should be contained in the table scan result are contained.", ixResult.containsAll(indexValues.get(i)));
						assertTrue("The table scan operator returned values that should not have been returned.", indexValues.get(i).containsAll(ixResult));
						assertTrue("Size of the results does not match the size of the reference list.", ixResult.size() == indexValues.get(i).size());						
						ixResult.clear();
						indexValues.get(i).clear();
					}
